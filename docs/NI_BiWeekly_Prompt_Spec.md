...
## Issue Synthesis Prompt ("This Issue at a Glance") — v1.1

Each issue opens with a short AI-written synthesis: two paragraphs (this issue's dominant themes; what changed since the previous issue) plus 2–4 named cross-cutting themes linked to their papers. It is generated by **one additional model call per issue**, after all per-paper summaries are complete.

**Division of labour — narrative vs. numbers.** The model writes prose ONLY. Every number shown in the glance box — paper counts, deltas vs. the previous issue, leading/rising/cooling topics (measured in percentage points of issue share, so issues of different sizes compare fairly), flagged counts, theme sizes — is computed deterministically in Python (`compute_deltas`) and passed INTO the prompt as verified fact. The model is instructed to use these numbers and never derive its own.

**Constraints (inherited from the per-abstract prompt):**
- Input is strictly the per-paper summaries already generated under this spec's faithfulness rules, plus the computed numbers. No abstracts re-read, no outside knowledge.
- No speculation about where the field is heading; no recommendations; no clinical conclusions.
- Same banned-superlatives list, enforced with the same retry-on-violation loop.
- Two paragraphs maximum, 110 words each; plain language, ~8th-grade reading level, active voice.
- Themes must reference papers by their exact IDs; IDs not present in the issue are dropped in code.

**Continuity.** The previous issue's synthesis is included in the prompt so the narrative can note continuing or fading threads. The synthesis is stored in the issue's JSON archive (`"synthesis"` key) alongside the papers, so future features (hub trends, theme tracking) can reuse it without regeneration.

**Failure mode.** If synthesis generation fails, the issue publishes normally without the glance box — the synthesis is additive and never blocks an issue.

**Backfill.** `python3 ni_biweekly.py --resynthesize` (or the "Resynthesize Issues" GitHub Action) generates the synthesis for existing issues that lack one, in chronological order, and re-renders their pages. Issues that already have a synthesis are skipped unless `--force-resynth` is passed.
...
| 1.1 | 2026-07-30 | Added issue-level "At a Glance" synthesis (one extra model call per issue; numbers computed in code, narrative constrained to per-paper summaries). Existing per-paper prompts unchanged. | M. Topaz |
